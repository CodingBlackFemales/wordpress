...
wp @$environment db query "UPDATE wp_${blog_id}_learndash_pro_quiz_master SET disabled_answer_mark = 1"

for post_id in ${post_id_list[@]}; do
	echo "Updating quiz ID:${post_id}"
	wp @$environment post meta patch update ${post_id} _sfwd-quiz sfwd-quiz_disabledAnswerMark true --url=${url} --format=json
done
