<?php
/**
 * Quiz Statistics Questions OpenAPI Documentation.
 *
 * Provides OpenAPI specification for quiz statistics questions endpoints.
...
/**
 * Quiz Statistics Questions OpenAPI Documentation Endpoint.
 *
 * @since 5.0.0
 */
class Quiz_Statistics_Questions extends LDLMS_V2_Endpoint {
	/**
	...
	public function get_response_schema( string $path = '', string $method = '' ): array {
		...
		if ( $this->determine_route_type( $route_path, 'sfwd-quiz/{quiz}/statistics/{statistic}/questions' ) === 'singular' ) {
			return [
				'$ref' => '#/components/schemas/LDLMS_v2_Quiz_Statistic_Question',
			];
		}
	...
	protected function get_routes(): array {
		...
		$statistics_questions_endpoint = LearnDash_Settings_Section::get_section_setting( 'LearnDash_Settings_Section_General_REST_API', 'quizzes-statistics-questions_v2' );

		return $this->discover_routes(
			trailingslashit( $quizzes_endpoint ) . '(?P<quiz>[\d]+)/' . $statistics_endpoint . '/(?P<statistic>[\d]+)/' . $statistics_questions_endpoint,
			[ 'collection', 'singular' ]
		);
	}
	...
	protected function get_method_summary( string $method, string $route_type = 'collection' ): string {
		$summaries = [
			'collection' => [
				'GET' => sprintf(
					...
					__( 'Get %1$s statistics for a specific %2$s', 'learndash' ),
			...
			'singular'   => [
				'GET' => sprintf(
					// translators: %1$s: question label. %2$s: quiz label.
					__( 'Get %1$s statistics for a specific %2$s and %1$s', 'learndash' ),
					learndash_get_custom_label_lower( LDLMS_Post_Types::QUESTION ),
					learndash_get_custom_label_lower( LDLMS_Post_Types::QUIZ ),
				),
			],
		...
		return $summaries[ $route_type ][ $method ]
			?? sprintf(
				// translators: %1$s: question label. %2$s: quiz label.
				__( '%1$s statistics %2$s operation', 'learndash' ),
				learndash_get_custom_label_lower( LDLMS_Post_Types::QUESTION ),
				learndash_get_custom_label_lower( LDLMS_Post_Types::QUIZ ),
			);
	}
	...
	protected function get_method_description( string $method, string $route_type = 'collection' ): string {
		$descriptions = [
			'collection' => [
				'GET' => sprintf(
					// translators: %1$s: question label. %2$s: quiz label.
					__( 'Retrieves the %1$s statistics for a specific %2$s statistic.', 'learndash' ),
					learndash_get_custom_label_lower( LDLMS_Post_Types::QUESTION ),
					learndash_get_custom_label_lower( LDLMS_Post_Types::QUIZ ),
				),
			],
			'singular'   => [
				'GET' => sprintf(
					// translators: %1$s: question label. %2$s: quiz label.
					__( 'Retrieves a specific %1$s statistics for a %2$s statistic and %1$s.', 'learndash' ),
					learndash_get_custom_label_lower( LDLMS_Post_Types::QUESTION ),
					learndash_get_custom_label_lower( LDLMS_Post_Types::QUIZ ),
				),
			],
		...
		return $descriptions[ $route_type ][ $method ] ?? sprintf(
			// translators: %1$s: question label. %2$s: quiz label.
			__( 'Performs %1$s operations on %2$s statistics.', 'learndash' ),
			learndash_get_custom_label_lower( LDLMS_Post_Types::QUESTION ),
			learndash_get_custom_label_lower( LDLMS_Post_Types::QUIZ ),
		);
	}
