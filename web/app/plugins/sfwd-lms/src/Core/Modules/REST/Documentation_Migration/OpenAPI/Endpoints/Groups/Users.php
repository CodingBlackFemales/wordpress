<?php
/**
 * Group Users OpenAPI Documentation.
 *
 * Provides OpenAPI specification for group users endpoints.
 * Currently based on V2 REST API: https://developers.learndash.com/rest-api/v2/v2-group-users/.
 *
 * @since 5.0.0
 *
...
/**
 * Group Users OpenAPI Documentation Endpoint.
 *
 * @since 5.0.0
 */
class Users extends LDLMS_V2_Endpoint {
	...
	public function get_response_schema( string $path = '', string $method = '' ): array {
		if ( $method !== WP_REST_Server::READABLE ) {
			return [
				'type'        => 'array',
				'description' => __( 'An array of objects for each processed user.', 'learndash' ),
				'items'       => [
					'properties' => [
						'user_id' => [
							'type'        => 'integer',
							'description' => __( 'The ID of the processed user.', 'learndash' ),
							'example'     => 123,
						],
						'status'  => [
							'type'        => 'string',
						...
						'message' => [
							...
							'example'     => $method === WP_REST_Server::DELETABLE
								? sprintf(
									...
									__( 'User unenroll from %s success.', 'learndash' ),
									learndash_get_custom_label_lower( 'group' )
								)
								: sprintf(
									// translators: %s: singular group label.
									__( 'User enrolled in %s success.', 'learndash' ),
									learndash_get_custom_label_lower( 'group' )
								),
						],
					],
	...
	protected function get_routes(): array {
		...
		$users_endpoint  = LearnDash_Settings_Section::get_section_setting( 'LearnDash_Settings_Section_General_REST_API', 'groups-users_v2' );

		return $this->discover_routes(
			trailingslashit( $groups_endpoint ) . '(?P<id>[\d]+)/' . $users_endpoint,
			[ 'collection' ]
		);
	}
	...
	protected function get_method_summary( string $method, string $route_type = 'collection' ): string {
		...
		return $summaries[ $route_type ][ $method ]
			?? sprintf(
				// translators: singular group label.
				__( '%1$s users operation', 'learndash' ),
	...
	protected function get_method_description( string $method, string $route_type = 'collection' ): string {
		...
		return $descriptions[ $route_type ][ $method ] ?? sprintf(
			// translators: singular group label.
			__( 'Performs operations on %s users.', 'learndash' ),
			learndash_get_custom_label_lower( 'group' )
		);
	}
	...
	protected function get_tags(): array {
		return [ sprintf( '%s-users', learndash_get_custom_label_lower( 'group' ) ) ];
	}
}
