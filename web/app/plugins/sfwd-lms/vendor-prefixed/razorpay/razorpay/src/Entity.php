...
class Entity extends Resource implements ArrayableInterface
{
    ...
    protected function validateIdPresence($id)
    {
        ...
        $code = \StellarWP\Learndash\Razorpay\Api\Errors\ErrorCode::BAD_REQUEST_ERROR;
    ...
    {
        return array('collection', 'payment', 'refund', 'order', 'customer', 'token', 'settlement');
    }
    ...
    public function setFile($attributes)
    {
        if (isset($attributes['file'])) {
            $attributes['file'] = new \CURLFILE($attributes['file'], mime_content_type($attributes['file']));
        }
        return $attributes;
    }
}