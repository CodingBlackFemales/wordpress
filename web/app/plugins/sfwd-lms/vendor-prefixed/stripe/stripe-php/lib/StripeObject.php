...
class StripeObject implements \ArrayAccess, \Countable, \JsonSerializable
{
    ...
    {
        list($id, $this->_retrieveOptions) = \StellarWP\Learndash\Stripe\Util\Util::normalizeId($id);
        $this->_opts = \StellarWP\Learndash\Stripe\Util\RequestOptions::parse($opts);
    ...
    public function __set($k, $v)
    {
        ...
        $this->_values[$k] = \StellarWP\Learndash\Stripe\Util\Util::convertToStripeObject($v, $this->_opts);
    ...
    public function updateAttributes($values, $opts = null, $dirty = true)
    {
        foreach ($values as $k => $v) {
            ...
            if ('metadata' === $k && \is_array($v)) {
                ...
                $this->_values[$k] = \StellarWP\Learndash\Stripe\Util\Util::convertToStripeObject($v, $opts);
            }
            if ($dirty) {
                $this->dirtyValue($this->_values[$k]);
            }
    ...
    public function serializeParamsValue($value, $original, $unsaved, $force, $key = null)
    {
        ...
        if (\is_array($value)) {
            if (\StellarWP\Learndash\Stripe\Util\Util::isList($value)) {
                ...
                return \StellarWP\Learndash\Stripe\Util\Util::convertToStripeObject($value, $this->_opts)->serializeParameters();
            }
    ...
    public function toArray()
    {
        ...
        return \array_reduce(\array_keys($this->_values), function ($acc, $k) use ($maybeToArray) {
            ...
            $v = $this->_values[$k];
            if (\StellarWP\Learndash\Stripe\Util\Util::isList($v)) {
