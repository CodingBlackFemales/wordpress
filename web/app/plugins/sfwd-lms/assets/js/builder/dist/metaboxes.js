...
if ('block' === window.learndash_builder_metaboxes.editor) {
  ...
  wp.data.subscribe(() => {
    ...
    let store,
      panels = {};
    if (semver.gte(window.learndash_builder_metaboxes.wp_version, '6.0.0')) {
      store = wp.data.select('core/preferences');
      panels = store.get('panels');
    } else {
      store = wp.data.select('core/edit-post');
      panels = store.getPreference('panels');
    }
  ...
  const quizTemplate = jQuery('#learndash-quiz-admin-data-handling-settings_templateSaveList');
  if (quizTemplate.length > 0) {
    const {
      isSavingPost
    } = wp.data.select('core/editor');
    let postSaved = true; // Start in a saved state.
    wp.data.subscribe(() => {
      if (isSavingPost()) {
        postSaved = false;
      } else if (!postSaved) {
        postSaved = true;
        let message = '';
        if (quizTemplate.val() === '0') {
          // Static value for new template.
          message = sprintf(__('%s template added.', 'learndash'), learndash.customLabel.get('quiz'));
        } else if (quizTemplate.val() > '0') {
          // Value is the ID of the template to update.
          message = sprintf(__('%s template updated.', 'learndash'), learndash.customLabel.get('quiz'));
        }
        if (!message) {
          return;
        }
        wp.data.dispatch('core/notices').createNotice('info', message, {
          id: 'learndash-quiz-template-notification',
          isDismissible: true,
          type: 'snackbar'
        });
        const quizTemplateName = jQuery('input[name="templateName"]');
        quizTemplate.val('-1'); // Reset to the original value.
        quizTemplate.trigger('change'); // Necessary to trigger the select2 events.
        quizTemplateName.val(''); // Reset the template name field value as well.
        quizTemplateName.trigger('change');
      }
    });
  }
} else {
