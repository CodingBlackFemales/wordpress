...
class WP_Job_Manager_Alerts_Shortcodes {
	...
	public function job_alerts_handler() {
		...
		try {
			if ( 'unsubscribe' === $this->action ) {
				...
				if ( ! $GLOBALS['job_manager_alerts']->verify_alert_token( $token, $alert_id, $user_id ) ) {
					throw new Exception( __( 'Invalid token', 'wp-job-manager-alerts' ) );
				}

				$this->delete_alert( $alert_id, $user_id );
					...
					case 'edit' :
						if ( isset( $_POST['submit-job-alert'] ) ) {
							...
							if ( $this->action == 'add_alert' ) {
								...
								$permission_checkbox_enabled = '1' === get_option( 'job_manager_permission_checkbox' );

								if ( $permission_checkbox_enabled && ! isset( $_POST[ 'alert_permission' ] ) ) {
								...
								if ( $permission_checkbox_enabled ) {
									update_post_meta( $alert_id, '_alert_permission_approved', '1' );
								}
							} else {
								$alert_id = absint( $_REQUEST['alert_id'] );
	 ...
	 */
	private function delete_alert( $alert_id, $user_id ) {
		$alert = get_post( $alert_id );

		// Check ownership.
		if ( absint( $alert->post_author ) !== $user_id ) {
			throw new Exception( __( 'Invalid Alert', 'wp-job-manager-alerts' ) );
		}
	...
	public function job_alerts( $atts ) {
		...
		if ( ! empty( $_GET['updated'] ) ) {
			$alert_message = '<div class="job-manager-message">' . __( 'Your alerts have been updated', 'wp-job-manager-alerts' ) . '</div>';
		}
