<div class="updated found_records">
	<?php if ($is_csv):?>
		<h3><?php printf(__('<span class="matches_count">%s</span> <strong>%s</strong> will be imported', 'wp-all-import-pro'), $node_list_count, _n('row', 'rows', $node_list_count, 'wp-all-import-pro')); ?></h3>
	<?php else:?>
		<h3><?php printf(__('<span class="matches_count">%s</span> <strong>%s</strong> %s will be imported', 'wp-all-import-pro'), $node_list_count, PMXI_Plugin::$session->source['root_element'], _n('element', 'elements', $node_list_count, 'wp-all-import-pro')); ?></h3>
	<?php endif; ?>
	<h4><?php _e('Click an element to select it, or scroll down to add filtering options.', 'wp-all-import-pro'); ?></h4>
