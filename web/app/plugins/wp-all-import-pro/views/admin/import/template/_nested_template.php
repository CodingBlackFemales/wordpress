<?php $custom_type = wp_all_import_custom_type( $post_type ); ?>
					...
					<td>						
							...
							<ul>
								<?php if ( ! empty($post['nested_files'])): ?>
									<?php 
										...
										?>
										<li rel="<?php echo $key;?>"><?php echo $file;?> <a href="javascript:void(0);" class="unmerge"><?php _e('remove', 'wp-all-import-pro'); ?></a></li>
										<?php
									}?>
								<?php endif; ?>
							</ul>
						...
						<div class="add_nested_file">
							...
							<div class="file-type-options">
								<label><?php _e('Specify the URL of the nested file to use.', 'wp-all-import-pro'); ?></label>
								<input type="text" class="regular-text" name="nested_url" value="" style="width:100%; line-height:20px;" placeholder="http(s)://"/>
							</div>

							<a rel="parse" href="javascript:void(0);" class="parse"><?php _e('Add', 'wp-all-import-pro'); ?></a>
						</div>											
					</td>
				</tr>							
			</table>
