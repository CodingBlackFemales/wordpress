...
final class Block {
	...
	public static function register_blocks() {
		...
		$blocks = apply_filters( '_gutenberg_blocks', array() );
	...
	public static function register( $block = '', $args = array() ) {
		...
		// Set editor styles.
		$editor_style = 'cbf-academy-' . $block_name . '-blocks';
		if ( ! empty( $args['editor_style'] ) ) {
			$editor_style = $args['editor_style'];
		}

		register_block_type(
			'cbf-academy/' . $block_name,
			array(
				'style'         => $styles,
				'script'        => $script,
				'editor_style'  => $editor_style,
				'editor_script' => 'cbf-academy-' . $block_name . '-blocks',
			)
		);
	}
}
