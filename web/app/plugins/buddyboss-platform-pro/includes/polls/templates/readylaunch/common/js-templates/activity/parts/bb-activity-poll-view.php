<?php
/**
 * ReadyLaunch - The template for displaying activity poll view.
 *
 * @since 2.7.50
 *
 * @version 1.0.0
					...
					<div class="bb-activity-poll-action-options">
						<# if (
							...
							<a href="#" class="bb-activity-poll-action-option bb-activity-poll-action-edit">
								<span class="screen-reader-text"><?php esc_html_e( 'Edit Poll', 'buddyboss-pro' ); ?></span>
								<span><i class="bb-icons-rl-pencil-simple"></i> <?php esc_html_e( 'Edit', 'buddyboss-pro' ); ?></span>
							</a>
						<# } #>
						<a href="#" class="bb-activity-poll-action-option bb-activity-poll-action-delete">
							<span class="screen-reader-text"><?php esc_html_e( 'Delete Poll', 'buddyboss-pro' ); ?></span>
							<span><i class="bb-icons-rl-trash"></i> <?php esc_html_e( 'Delete', 'buddyboss-pro' ); ?></span>
						</a>
					</div>
				</div>
							...
							<div class="bb-activity-poll-option">
								...
								<div class="bp-{{field_name}}-wrap bb-option-field-wrap">
									...
									if ( ! poll_closed ) {
										...
										<input type="{{field_name}}" class="bb-rl-styled-{{field_name}} bb-option-input-wrap" id="bb-activity-poll-option-{{index}}" name="bb-activity-poll-option-{{index}}" value="{{option.option_title}}" data-opt_id="<# if ( option.id ) { #>{{option.id}}<# } else { #> {{index}} <# } #>"/>
									<# } #>
