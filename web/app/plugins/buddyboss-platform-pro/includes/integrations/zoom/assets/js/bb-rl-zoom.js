...
( function( exports, $ ) {
	...
	bp.Zoom = {
		...
		addListeners: function () {
			...
			$( document ).on( 'click', '.bb-rl-close-create-meeting-form', this.backToMeetingList.bind( this ) );
			...
			// Catch form submit inside modal
			$( document ).on( 'click', '#bp-zoom-meeting-form-submit', this.updateMeeting.bind( this ) );
			...
			$( document ).on( 'click', '.bb-rl-close-create-webinar-form', this.backToWebinarsList.bind( this ) );
			...
			if ( typeof this.mask_meeting_id === 'function' ) {
				this.mask_meeting_id();
			}
			if ( typeof this.mask_webinar_id === 'function' ) {
				...
				meeting_wrapper.find( '#bp-zoom-meeting-timezone' ).select2(
					{
						...
						dropdownParent: meeting_wrapper.find( '#bp-zoom-meeting-timezone' ).parent()
					}
				);
				
				// Apply CSS classes after initialization
				meeting_wrapper.find( '#bp-zoom-meeting-timezone' ).next( '.select2-container' ).addClass( 'bb-select-container' );
				
				// Add class to dropdown when it opens
				meeting_wrapper.find( '#bp-zoom-meeting-timezone' ).on(
					'select2:open',
					function () {
						$( '.select2-dropdown' ).addClass( 'bb-select-dropdown' );
					}
				);
			}
			...
			if (typeof jQuery.fn.datetimepicker !== 'undefined') {
				...
				var webinar_options = {
					...
					onKeyPress: function( cep ) {
						var masks = [ 'PX:YZ', 'PQ:YZ' ];
						var mask  = ( cep.length > 1 && cep.substr( 0,1 ) > 0 ) ? masks[1] : masks[0];
						webinar_wrapper.find( '#bp-zoom-webinar-start-time' ).mask( mask, webinar_options );
					}
				};
				...
				webinar_wrapper.find( '#bp-zoom-webinar-timezone' ).select2(
					{
						...
						closeOnSelect: true,
						language: this.select2_laguage_text,
						dropdownParent: webinar_wrapper.find( '#bp-zoom-webinar-timezone' ).parent()
					}
				);
				
				// Apply CSS classes after initialization
				webinar_wrapper.find( '#bp-zoom-webinar-timezone' ).next( '.select2-container' ).addClass( 'bb-select-container' );
				
				// Add class to dropdown when it opens
				webinar_wrapper.find( '#bp-zoom-webinar-timezone' ).on(
					'select2:open',
					function () {
						$( '.select2-dropdown' ).addClass( 'bb-select-dropdown' );
					}
		...
		// Helper function to initialize date picker fields
		initMeetingFormDatePickers: function( container ) {
			$( container ).find( '#bp-zoom-meeting-start-date' ).datetimepicker(
				{
					format: 'Y-m-d',
					timepicker: false,
					mask: true,
					...
					scrollInput: false,
					onSelectDate: function (date,element) {
						$( container ).find( '#bp-zoom-meeting-end-date-time' ).datetimepicker(
							{
								minDate: element.val(),
							}
						);
			...
			$( container ).find( '#bp-zoom-meeting-start-time' ).datetimepicker(
				{
					format: 'h:i',
					formatTime:	'h:i',
					datepicker: false,
			...
			var options = {
				...
				onKeyPress: function( cep, e, field, options ) {
					var masks = [ 'PX:YZ', 'PQ:YZ' ];
					var mask  = ( cep.length > 1 && cep.substr( 0,1 ) > 0 ) ? masks[1] : masks[0];
					$( container ).find( '#bp-zoom-meeting-start-time' ).mask( mask, options );
				}
			};

			$( container ).find( '#bp-zoom-meeting-start-time' ).mask( 'PX:YZ', options );

			if (typeof jQuery.fn.select2 !== 'undefined') {
				$( container ).find( '#bp-zoom-meeting-timezone' ).select2(
					{
						minimumInputLength: 0,
						closeOnSelect: true,
						language: this.select2_laguage_text,
						dropdownParent: $( container ).find( '#bp-zoom-meeting-timezone' ).parent()
					}
				);

				// Apply CSS classes after initialization
				$( container ).find( '#bp-zoom-meeting-timezone' ).next( '.select2-container' ).addClass( 'bb-select-container' );
				
				// Add class to dropdown when it opens
				$( container ).find( '#bp-zoom-meeting-timezone' ).on(
					'select2:open',
					function () {
						$( '.select2-dropdown' ).addClass( 'bb-select-dropdown' );
					}
		...
		loadEditMeeting: function( e ) {
			...
			// Load the meeting data via AJAX
		...
		updateMeeting: function (e) {
			...
			_this.parents( '#bp-zoom-single-meeting-wrapper' ).find( '.bb-rl-notice' ).remove();
			...
			this.bp_zoom_ajax = $.ajax(
				{
					...
					success: function (response) {
						...
						var success_html = '<aside class="bb-rl-notice bb-rl-notice--alt bb-rl-notice--success">' +
						'<span class="bb-icons" aria-hidden="true"></span>' +
						'<p>' + (response.data.message || 'Meeting updated successfully.') + '</p>' +
						'</aside>';
						
						
						_this.parents( '#bp-zoom-single-meeting-wrapper' ).prepend( success_html );
						$( 'html, body' ).animate( { scrollTop: $( '#bp-zoom-single-meeting-wrapper' ).offset().top - 100 }, 500 );
							...
							if ( response.data.errors ) {
								for ( var er in response.data.errors ) {
									error_html = '<aside class="bb-rl-notice bb-rl-notice--alt bb-rl-notice--error">' +
									'<span class="bb-icons" aria-hidden="true"></span>' +
									'<p>' + response.data.errors[er].message + '</p>' +
									'</aside>';
									_this.parents( '#bp-zoom-single-meeting-wrapper' ).prepend( error_html );
								}
		...
		updateWebinar: function (e) {
			...
			_this.parents( '#bp-zoom-single-webinar-wrapper' ).find( '.bb-rl-notice' ).remove();
			...
			this.bp_zoom_ajax = $.ajax(
				{
					...
					success: function (response) {
						...
						if (response.success) {
							...
							var success_html = '<aside class="bb-rl-notice bb-rl-notice--alt bb-rl-notice--success">' +
							'<span class="bb-icons" aria-hidden="true"></span>' +
							'<p>' + (response.data.message || 'Webinar updated successfully.') + '</p>' +
							'</aside>';
							
							_this.parents( '#bp-zoom-single-webinar-wrapper' ).prepend(success_html);
							...
							if ( response.data.errors ) {
								for ( var er in response.data.errors ) {
									...
									'<p>' + response.data.errors[er].message + '</p>' +
									'</aside>';
									_this.parents( '#bp-zoom-single-webinar-wrapper' ).prepend( error_html );
								}
							} else if ( response.data.error ) {
								error_html = '<aside class="bb-rl-notice bb-rl-notice--alt bb-rl-notice--error">' +
								'<span class="bb-icons" aria-hidden="true"></span>' +
