...
namespace WPMailSMTP\Vendor\Aws\Configuration;
...
class ConfigurationResolver
{
    ...
    public static function ini($key, $expectedType, $profile = null, $filename = null, $options = [])
    {
        ...
        $data = @\WPMailSMTP\Vendor\Aws\parse_ini_file($filename, \true, \INI_SCANNER_NORMAL);
    ...
    {
        if ($type === 'bool' && !\is_null($convertedValue = \WPMailSMTP\Vendor\Aws\boolean_value($value))) {
    ...
    private static function retrieveValueFromIniSubsection($data, $profile, $filename, $expectedType, $options)
    {
        ...
        $services_section = \WPMailSMTP\Vendor\Aws\parse_ini_section_with_subsections($filename, "services {$data[$profile]['services']}");
