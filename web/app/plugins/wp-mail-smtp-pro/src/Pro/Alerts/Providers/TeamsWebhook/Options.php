...
/**
 * Class Options. Microsoft Teams Incoming Webhook settings.
 *
 * @since 4.1.0
 */
	...
	public function __construct() {
		...
		$description = wp_kses(
			sprintf(
				...
				__( 'Paste in the Microsoft Teams webhook URL you’d like to use to receive alerts when email sending fails. <a href="%s" target="_blank" rel="noopener noreferrer">Read our documentation on setting up Microsoft Teams alerts</a>.', 'wp-mail-smtp-pro' ),
				esc_url(
					wp_mail_smtp()->get_utm_url(
						'https://wpmailsmtp.com/docs/setting-up-email-alerts/#msteams',
						[
							'medium'  => 'Alerts Settings',
							'content' => 'Microsoft Teams Documentation',
						]
					)
				)
			),
		...
		parent::__construct(
			[
				'slug'                => self::SLUG,
				'title'               => esc_html__( 'Microsoft Teams', 'wp-mail-smtp-pro' ),
				'description'         => $description,
				'add_connection_text' => esc_html__( 'Add Another Webhook', 'wp-mail-smtp-pro' ),
			]
		);
	}
				...
				<div class="wp-mail-smtp-setting-field">
					...
					<?php if ( $this->options->is_const_defined( $group, 'connections' ) ) : ?>
						<p class="desc">
							<?php
							//phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							echo $this->options->get_const_set_message( 'WPMS_ALERT_TEAMS_WEBHOOK_URL' );
							?>
						</p>
					<?php endif; ?>
				</div>
