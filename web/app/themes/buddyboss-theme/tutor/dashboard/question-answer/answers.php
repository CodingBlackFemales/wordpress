...
$profile_url = bb_theme_enable_tutorlms_override() ? trailingslashit( bp_core_get_user_domain( $question->user_id ) . bp_get_profile_slug() ) : tutor_utils()->profile_url( $question->user_id, true );

?>
<h2><?php esc_html_e( 'Answer', 'buddyboss-theme' ); ?></h2>
		...
		<div class="tutor_original_question">
			<div class="tutor-question-wrap">
				<div class="question-top-meta">
					<div class="tutor-question-avater <?php echo bb_theme_enable_tutorlms_override() ? 'bb-tutor-avatar' : ''; ?>">
						<a href="<?php echo esc_url( $profile_url ); ?>">
							<?php
							if ( bb_theme_enable_tutorlms_override() ) {
								echo get_avatar( $question->user_id, 50 );
							} else {
								echo wp_kses(
									tutor_utils()->get_tutor_avatar( $question->user_id ),
					...
					<p class="review-meta">
						...
						<?php echo esc_html( wp_sprintf( __( '%s ago', 'buddyboss-theme' ), human_time_diff( strtotime( $question->comment_date_gmt ) ) ) ); ?>
					</span>
					</p>
				</div>
		...
		<div class="tutor_admin_answers_list_wrap">
							...
							<div class="tutor-question-wrap">
								<div class="question-top-meta">
									...
									<p class="review-meta">
										...
										<span class="tutor-text-mute">
										<?php echo esc_html( wp_sprintf( __( '%s ago', 'buddyboss-theme' ), human_time_diff( strtotime( $answer->comment_date_gmt ) ) ) ); ?>
										</span>
									</p>
								</div>
			...
			<div class="tutor-add-question-wrap">
				<form action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" id="tutor_admin_answer_form" method="post">
					...
					<div class="tutor-form-group">
						<button type="submit" class="tutor-button tutor-button-primary" name="tutor_question_search_btn"><?php esc_html_e( 'Reply', 'buddyboss-theme' ); ?> </button>
					</div>
				</form>
			</div>
		</div>
