...
$footer_links = array(
	array(
		'title'      => $is_instructor ? __( 'My Courses', 'buddyboss-theme' ) : __( 'Dashboard', 'buddyboss-theme' ),
		'url'        => $footer_url_1,
		'is_active'  => $footer_url_1 == $current_url,
		'icon_class' => 'ttr tutor-icon-dashboard',
	),
	array(
		'title'      => $is_instructor ? __( 'Q&A', 'buddyboss-theme' ) : __( 'Quiz Attempts', 'buddyboss-theme' ),
		'url'        => $footer_url_2,
		'is_active'  => $footer_url_2 == $current_url,
		'icon_class' => $is_instructor ? 'ttr  tutor-icon-question' : 'ttr tutor-icon-quiz-attempt',
	),
	array(
		'title'      => __( 'Menu', 'buddyboss-theme' ),
		'url'        => '#',
		'is_active'  => false,
	...
	<div class="tutor-container">
		<div class="tutor-row tutor-d-flex tutor-justify-between tutor-frontend-dashboard-header">
			<div class="tutor-header-left-side tutor-dashboard-header tutor-col-md-6 tutor-d-flex tutor-align-center" style="border: none;">
				<div class="tutor-dashboard-header-avatar <?php echo bb_theme_enable_tutorlms_override() ? 'bb-tutor-avatar' : ''; ?>">
					<?php
				...
				<div class="tutor-user-info tutor-ml-24">
					...
					$display_name      = function_exists( 'bp_core_get_user_displayname' ) ? bp_core_get_user_displayname( $user->ID ) : $user->display_name;
					if ( current_user_can( tutor()->instructor_role ) ) {
						...
						<div class="tutor-dashboard-header-display-name tutor-color-black">
							<div class="tutor-fs-5 tutor-dashboard-header-greetings">
								<?php esc_html_e( 'Hello', 'buddyboss-theme' ); ?>,
							</div>
				...
				<div class="tutor-d-flex tutor-align-center">
					...
					if ( tutor_utils()->get_option( 'enable_become_instructor_btn' ) ) {
						...
						<a id="tutor-become-instructor-button" class="tutor-btn tutor-btn-outline-primary" href="<?php echo esc_url( tutor_utils()->instructor_register_url() ); ?>">
							<i class="tutor-icon-user-bold"></i> &nbsp; <?php esc_html_e( 'Become an instructor', 'buddyboss-theme' ); ?>
						</a>
						<?php
					}
					...
					if ( current_user_can( tutor()->instructor_role ) ) {
						...
						if ( function_exists( 'tutor_pro' ) ) :
							...
							<a href="<?php echo esc_url( admin_url( "post-new.php?post_type=$course_type" ) ); ?>" class="tutor-btn tutor-btn-outline-primary">
								...
								<?php esc_html_e( 'Create a New Course', 'buddyboss-theme' ); ?>
							</a>
					<?php endif; ?>
						<?php
						...
						esc_html__( 'Your Application is pending as of', 'buddyboss-theme' ), ' <b>', esc_html( $on ), '</b>',
						'</span>';
				...
				<ul class="tutor-dashboard-permalinks">
					...
					foreach ( $dashboard_pages as $dashboard_key => $dashboard_page ) {
						...
						if ( is_array( $dashboard_page ) ) {
							...
							$menu_icon_name = tutor_utils()->array_get( 'icon', $dashboard_page, ( $dashboard_page['icon'] ?? '' ) );
							if ( $menu_icon_name ) {
