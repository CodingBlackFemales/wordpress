...
$courses_label     = esc_html__( 'Courses', 'buddyboss-theme' );
$course_label      = esc_html__( 'Course', 'buddyboss-theme' );
?>
<div id="lifterlms-content" class="lifterlms-course-list">
	<form id="bb-courses-directory-form" class="bb-courses-directory" method="get" action="" data-courses_label='<?php echo esc_attr( $courses_label ); ?>' data-course_label='<?php echo esc_attr( $course_label ); ?>' >
		...
		<ul class="component-navigation courses-nav">
				<?php
				$navs = array(
					'all' => '<div class="bb-component-nav-item-point">' . esc_html__( 'All Courses', 'buddyboss-theme' ) . '</div>',
				);

				if ( is_user_logged_in() ) {
					$navs['my-courses'] = '<div class="bb-component-nav-item-point">' . esc_html__( 'My Courses', 'buddyboss-theme' ) . '</div>';
				}
		...
		if ( isset( $_GET['type'] ) && 'my-courses' === $_GET['type'] ) {
			...
			$count = ! empty( $all_query->posts ) ? $all_query->found_posts : 0;

		} else {
			global $wp_query;
			$count = ! empty( $wp_query->posts ) ? $wp_query->found_posts : 0;
		}
		...
		<div class="ld-secondary-header ld-secondary-header--llms">
			<div class="bb-secondary-list-tabs flex align-items-center" id="subnav" aria-label="Members directory secondary navigation" role="navigation">
				...
				<div class="grid-filters" data-view="llms-course">
					<a href="#" class="layout-view layout-view-course layout-grid-view bp-tooltip <?php echo esc_attr( $class_grid_active ); ?>" data-view="grid" data-bp-tooltip-pos="up" data-bp-tooltip="<?php esc_attr_e( 'Grid View', 'buddyboss-theme' ); ?>" aria-label="<?php esc_attr_e( 'Grid View', 'buddyboss-theme' ); ?>"><i class="dashicons dashicons-screenoptions" aria-hidden="true"></i>
					</a>

					<a href="#" class="layout-view layout-view-course layout-list-view bp-tooltip <?php echo esc_attr( $class_list_active ); ?>" data-view="list" data-bp-tooltip-pos="up" data-bp-tooltip="<?php esc_attr_e( 'List View', 'buddyboss-theme' ); ?>" aria-label="<?php esc_attr_e( 'List View', 'buddyboss-theme' ); ?>"><i class="dashicons dashicons-menu" aria-hidden="true"></i>
					</a>
				</div>
