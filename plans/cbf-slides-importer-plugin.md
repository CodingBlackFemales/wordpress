...
| R29 | **A title clash with another course is reported as fixable by Overwrite.** The importer matches an existing post by title across the whole site, not within the target course, so two courses cannot both hold an "Introduction to Git" — and following the report's advice would rewrite the other course's lesson and move it into this one | Medium (4 rows in the first 29) | High if acted on — damages a course nobody was migrating | Developer | Name the owning course in the report and tell the editor to retitle the row; recommend Overwrite only when the clash is one this course already owns. The site-wide match itself is left alone, because the existing CLI pipeline depends on it |
