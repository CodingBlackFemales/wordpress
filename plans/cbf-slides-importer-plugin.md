...
| A10 | LearnDash section headings are **virtual markers**, not posts: a JSON array in the course's `course_sections` post meta, each entry `{ID, order, post_title, type: 'section-heading'}`, where **`order` is an index into the part-built display list, not into the lesson list** — LearnDash splices the headings in one at a time, so by the time the Nth is inserted the N before it already occupy slots (`order = lessons_before + headings_before`, see R30) | ✅ **Verified** — read from `LDLMS_Course_Steps::steps_grouped_sections()` and `set_section_headings()`; corroborated by the [LearnDash sections documentation](https://docs.nexcess.com/software/learndash/course-sections/). **P8.6a: `ID` is a millisecond Unix timestamp** assigned client-side at creation, confirmed against live data (`{"order":0,"ID":1631808942506,"post_title":"Test Driven Development",...}` = 16 Sep 2021) | Section creation would need a different mechanism | — |
...
| R31 | **An edited script keeps its cached URL.** Assets are served with a one-year `max-age` and versioned by the plugin's `VERSION` constant, which only moves at release — so a fixed JS file goes on being served from the browser cache, and the fix looks like it did not work | Medium — certain to recur during development | Medium — time lost debugging code that is already correct | Developer | Append the file's `filemtime` to the version, so the query argument changes whenever the file does while still naming the release it came from |
| R30 | **`order` was read as an index into the lesson list, which is the natural reading and the wrong one.** LearnDash builds the displayed list by splicing each heading in turn — `foreach ( $sections as $s ) array_splice( $lessons, $s->order, 0, [ $s ] )` — so each splice shifts everything after it, and heading N must account for the N already inserted. Writing bare lesson counts drifted every heading one position earlier per heading before it, moving the last lesson of each section into the next: **28 of 109 lessons misplaced** in the first full migration | High (shipped, and found by the editor rather than by any check) | High — silently wrong course structure | Developer | `section_order( $lessons_before, $headings_before )` returns `$lessons_before + $headings_before`, and `open_sections_up_to()` subtracts the same offset when reading membership back. **The reason nothing caught this is the lesson worth keeping**: verification re-derived the grouping with the same wrong rule used to write it, so the course confirmed as correct. `SectionOrderTest` now tests against a replica of LearnDash's splice instead, and asserts that the old rule misplaces content |
| R28 | **A batch leaves the course visibly wrong while it runs.** Headings are created before any lesson exists, so they sit at indices 0..n and the builder shows one heading per lesson with the remainder piled under the last — indistinguishable from a placement bug, and permanent if the batch is cancelled | High (reported from the first real run) | Medium — no data lost, but the editor cannot tell progress from failure | Developer | Run the placement pass after every row, and again on `finish()` and `cancel()` |
| R29 | **A title clash was treated as a collision when it is actually shared content.** The importer matches an existing post by title across the whole site, and the row was skipped — leaving a gap in the course. But **shared course steps are enabled on this site** (A14), so a session is not owned by one course: several bootcamps legitimately teach the same "Introduction to Git" | Medium (4 rows in the first 29, and shared titles are common across CBF's bootcamps) | High — sessions silently missing from a migrated course | Developer | With shared steps on and Overwrite off, add the existing post to the course as a shared step and report the row as **reused**, naming the courses it already belongs to. `set_steps()` performs the attach, which is precisely what the course builder does when a step is shared. Overwrite still wins where the editor asked for it; where shared steps are off, the row is skipped with a message that names the owning course and warns against Overwrite |
...
| R32 | **No way to see or change which Google account is connected.** Authorisation was a one-way door: the consent link appeared only while unconnected, `revoke_token()` was never reachable from the UI, and nothing named the account in use. A staging deployment was authorised as the wrong account and it surfaced only when content did not appear | High (happened) | Medium — imports run against the wrong Drive, or an unwanted grant stands indefinitely | Developer | A panel on the Import tab names the connected account from `drive.readonly`'s `about?fields=user`, with **Use a different account** and **Disconnect**. Consent now sends `prompt=select_account consent`, without which Google silently re-uses the signed-in account and a wrong choice cannot be undone. `revoke_token()` withdraws the grant at Google as well as deleting the stored copy — deleting locally only made this site forget the account while its access stood |
| R25 | **The Google API client cannot build a transport in this environment.** google/apiclient 2.19 accepts Guzzle 6 or 7 only; Bedrock's root vendor ships **Guzzle 8.0.2** (since 2026-07-30) and wins the autoloader race against the copy bundled with the plugin, so every call through `DriveService` throws `LogicException: Could not find supported version of Guzzle` before reaching the network | High — was live, undetected | High — bulk pre-flight could resolve no file at all | Developer | Read metadata over `wp_remote_get` with the Bearer token instead of the API client, mirroring the fallback the export path already had. This was invisible until bulk because the single-file flow gets its MIME type from the Drive Picker and the export path silently falls back — so only the metadata call, which bulk depends on entirely, had no escape route. **It reached further than that**: `fetchAccessTokenWithRefreshToken()` and `fetchAccessTokenWithAuthCode()` are on the same broken path, so no token could be refreshed and no authorisation completed. Nothing noticed because both only happen once an access token passes an hour old, and every run so far finished inside that. The token endpoint now goes over `wp_remote_post` as well |
| R26 | **Parsing exhausts the memory a cron request is allowed.** A 7.2 MB, 42-slide PPTX peaks at **452 MB** — PhpPresentation holds every slide's object graph and every embedded image at once — against the 256 MB `WP_MEMORY_LIMIT` gives a cron request | High (hit on the first row of the first real run) | High — the worker dies mid-parse | Developer | `wp_raise_memory_limit( 'cbf_si_job' )` lifts the ceiling to 1 GB for the job, filterable via `cbf_si_job_memory_limit`. Sequential execution means one parse at a time, so peak usage is bounded by the largest single document |
| R27 | **A fatal error freezes a batch rather than failing a row.** `run()` catches exceptions; memory exhaustion, a timeout and a library segfault are not exceptions, so the job stays in `parsing` with no error against it and the batch waits on a row that will never report | High — the mechanism by which R26 surfaced | High — a whole migration stalls on one bad document | Developer | A shutdown handler registered at the start of each job inspects `error_get_last()` and marks the row failed with an editor-facing reason, releasing the batch. A 512 KB buffer is reserved up front and freed in the handler, so it still has room to run after an out-of-memory kill. The Janitor abandons a job after `MAX_ATTEMPTS` stalls, so a worker killed without running its handler cannot cycle forever |
...
Because a heading's position is expressed as an index rather than a relationship (A10), placement is applied by rewriting the whole course order in one sweep: collect the created lesson IDs per heading, then reorder.

That sweep runs **after every row**, not only at the end. It was originally deferred to batch completion (R15), on the reasoning that one pass is cheaper than many. In practice that left the course builder scrambled for the length of a run — the headings are created up front against a course that may still be empty, so they occupy indices 0..n and every lesson lands under the wrong one until the pass runs — and a batch that was cancelled or abandoned never ran it at all, leaving the course that way permanently. Rows are sequential, so there is no interleaving for a single pass to protect against; the pass is idempotent, and re-running it per row keeps the course correct at every point a person might look at it. A final pass still runs at `finish()` and on `cancel()`, covering a row that reports after the batch has stopped.

**Reused content is placed the same way as created content.** With shared steps enabled, adding an existing session to a course *is* adding it to the step tree, so a reused row simply joins the placement list. `attach_missing()` puts any lesson in the sequence that the tree does not yet hold into it before reordering — which covers both a reused session and a tree read before this batch created its lessons. It refuses only when something in the sequence is not a lesson at all, since that means a caller has gone wrong and writing it would attach nonsense to the course.
...
- **The steps model is cached per request.** A tree read before the batch created its lessons is stale, and writing it back silently detaches everything created since. The reorder pass reloads with `LDLMS_Factory_Post::course_steps( $course_id, true )`, then `attach_missing()` puts anything absent from the tree into it rather than writing a tree that would drop it. The first version of this refused the write instead, which protected the lessons but left the course unordered — and made reusing a shared session impossible, because a shared session is by definition not in the tree yet.
...
Jobs in a batch run **strictly sequentially** — one in flight at a time. Each job's completion schedules the next. This is slower than parallel execution and deliberately so: it is what makes lesson ordering deterministic (R15), keeps Drive well inside rate limits (R17), and bounds peak memory to a single parse. The production CSV is expected to be roughly 120 rows, of which ~109 are importable. **Measured: a full 119-row run took about 25 minutes**, at the fast end of the 20–55 minute estimate. This is background work either way, but the duration has consequences worth designing for: the batch must survive WP-Cron ticks across that window, resume from the first unfinished row after a stale-job reset, and show live progress so an editor can tell a slow batch from a stalled one.
